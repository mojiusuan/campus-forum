...
创建 `/var/www/forum/scripts/backup-db.sh`：
...
```bash
# 检查CORS配置
# 确保后端 .env 中的 CORS_ORIGIN 包含前端域名

# 检查API URL
# 确保前端 .env.production 中的 VITE_API_URL 正确
```
...
- [ ] 后端API健康检查：`https://api.yourdomain.com/api/health`
- [ ] 前端页面正常访问：`https://yourdomain.com`
- [ ] 用户注册功能正常
- [ ] 用户登录功能正常
- [ ] 发帖功能正常
- [ ] 图片上传功能正常
...
# 2. 更新后端（构建需要完整依赖，不要用 --production）
cd backend
npm install
npx prisma generate
npx prisma migrate deploy
npx tsx prisma/seed.ts          # 可选：补全种子数据（如情感树洞、匿名用户）
npm run build
pm2 restart forum-api
...
npm install
npm run build
# 如果使用Nginx，静态文件已在 dist 目录，无需额外操作
# 如果使用OSS，需将 dist 内容重新上传
...
**详细分析请参考：** `docs/高并发架构优化方案.md`

### 快速优化措施（提升到5000-10000用户）

#### 1. 优化数据库连接池

已在 `backend/src/utils/db.ts` 中配置，可通过环境变量调整：
